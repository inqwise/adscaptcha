...
namespace Inqwise.AdsCaptcha.Handlers
{
    public class ImportHandler : BaseHandler, IHttpHandler
    {
        ...
        static readonly string TempFolderPath = ApplicationConfiguration.TempFolderPath.Value;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                int imageTypeId;
                if (!(int.TryParse(_request["imageTypeId"], out imageTypeId) && Enum.IsDefined(typeof(ImageType), imageTypeId)) || ImageType.Undefined == (ImageType)imageTypeId)
                {
                    imageTypeId = (int)ImageType.Temp;
                }

                string imageId = _request["imageId"];
                if (!string.IsNullOrEmpty(imageId))
                {
                    ImagesManager.RemoveTmp(_server.UrlDecode(imageId));
                }
                ...
                if(null == result)
                {
                    ...
                    if (fileInfo.ImageType == ImageType.Temp)
                    {
                        AdsCaptchaOperationResult addTempResult = ImagesManager.AddTemp(fileInfo);
                        if (addTempResult.HasError)
                        {
                            fileInfo.Error = addTempResult.Error;
                        }
                        else
                        {                            
                            imageId = _server.UrlEncode(fileInfo.RelativeFilePath);
                        }
                    }
                    else
                    {
                        ...
                        {
                            imageId = addResult.Value.ToString();
                        }
                    }
                }
