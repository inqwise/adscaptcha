...
namespace Inqwise.AdsCaptcha.Advertiser
{
    public partial class UploadImage : System.Web.UI.Page
    {
        ...
        private int mAdType = 16005;
        private const string SCRIPT_TEMPLATE = "<script type='text/javascript'>window.parent.photoUploadComplete('{0}', {1}, '{2}', '{3}', {4}, {5});</script>";
        private const string SLIDER_RUN_TEMPLATE = "<script type='text/javascript'>window.parent.ChangeAd('{0}', '{1}', {2}, '{3}', '{4}', {5}, {6});</script>";
        ...
        private void UploadPhoto()
        {
            ...
            {
                //script = string.Format(SCRIPT_TEMPLATE, "Required", "true", "", "", 0, 0);
                script = string.Format(SLIDER_RUN_TEMPLATE, "", "Required", "true", "", "", 0, 0);
            }
            else
            {
                ...
                if (!IsImageValid(uploadImage.PostedFile, ref errorMessage))
                {
                    ...
                    script = string.Format(SLIDER_RUN_TEMPLATE, "", errorMessage, "true", "", "", 0, 0);
                }
            }
            ...
            if (string.IsNullOrEmpty(script))
            {
                ...
                try
                {
                    ...
                    using (Bitmap bitmap = new Bitmap(uploadImage.PostedFile.InputStream, false))
                    {
                        ...
                        if (
                            mAdType == (int)AdType.SloganAndImage && (width > Constants.MAX_IMAGE_WIDTH || height > Constants.MAX_IMAGE_HEIGHT)
                            ||
                            mAdType == (int)AdType.Slide2Fit && (width > Constants.MAX_SLIDER_DIM)
                            )
                        {
                            switch (mAdType)
                            {
                                case (int)AdType.Slide2Fit:
                                    int minSize = (width > Constants.ALLOWED_SLIDER_DIM[1] ? Constants.ALLOWED_SLIDER_DIM[1] : Constants.MIN_SLIDER_DIM);
                                    percentW = minSize / (float)width;
                                    percentH = minSize / (float)height;
                                    break;
                                case (int)AdType.SloganAndImage:
                                default:
                                    percentW = Constants.MAX_IMAGE_WIDTH / (float)width;
                                    percentH = Constants.MAX_IMAGE_HEIGHT / (float)height;
                                    break;
                            }
                            ...
                            {
                                case (int)AdType.Slide2Fit:
                                    if (newWidth < Constants.MIN_SLIDER_DIM) newWidth = Constants.MIN_SLIDER_DIM;
                                    if (newHeight < Constants.MIN_SLIDER_DIM) newHeight = Constants.MIN_SLIDER_DIM;
                                    break;
                                case (int)AdType.SloganAndImage:
                                default:
                                    if (newWidth < Constants.MIN_IMAGE_WIDTH) newWidth = Constants.MIN_IMAGE_WIDTH;
                                    if (newHeight < Constants.MIN_IMAGE_HEIGHT) newHeight = Constants.MIN_IMAGE_HEIGHT;
                                    break;
                            }
                        }
                        */
                        ...
                        int tempAdId = RandomNumber(999999, 999999999);
                        uploader.UploadImage(-1, -1, tempAdId, width, height, ms.ToArray());
                        adId = tempAdId.ToString();
                        */
                    ...
                    script = string.Format(SLIDER_RUN_TEMPLATE, adId, "Image successfully uploaded", "false", fileUrl, fileName, width, height);
                }
                catch (Exception ex)
                {
                    Log.ErrorException("UploadPhoto : Unexpected error occured", ex);
                    //script = string.Format(SCRIPT_TEMPLATE, "Error occured, please try again", "true", "", "", 0, 0);
                    adId = string.Empty;
                    script = string.Format(SLIDER_RUN_TEMPLATE, "", "Error occured, please try again", "true", "", "", 0, 0);
        ...
        private bool ValidateImageDimensions(HttpPostedFile file)
        {
            using (Bitmap bitmap = new Bitmap(file.InputStream, false))
            {
                return AdBLL.IsDimensionValid(mAdType, bitmap.Width, bitmap.Height);
            }
        }
        ...
        private bool IsImageValid(HttpPostedFile file, ref string errorMessage)
        {
            ...
            {
                errorMessage = "File dimensions need to be 250X300; 150X180";
                return false;
            }
