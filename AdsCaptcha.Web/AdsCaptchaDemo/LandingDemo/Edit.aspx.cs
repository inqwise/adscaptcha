...
public partial class LandingDemo_Edit : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            {
                //LoadData(Convert.ToInt32(Request["did"]));
    ...
    /*
    private void LoadData(int demoid)
    {
        ddlDimentions.Enabled = true;
        using (AdsCaptcha_ImagesEntities ent = new AdsCaptcha_ImagesEntities())
        {
            ...
            if (demo != null)
            {
                lblName.Text = demo.DemoName;
                txtDesc.InnerText = demo.DemoDescription;
                txtClick.Text = demo.ClickUrl;
                txtLike.Text = demo.LikeUrl;

                btnPreview.CommandArgument = demo.DemoUrlName;

                var demoimg = demo.T_DEMOS_IMAGES.FirstOrDefault();
                if (demoimg != null)
                {
                    demoimg.T_IMAGESReference.Load();
                    if (demoimg.T_IMAGES.Width == 300)
                    {
                        ddlDimentions.SelectedIndex = 0;
                    }
                    else if (demoimg.T_IMAGES.Width == 180)
                    {
                        ddlDimentions.SelectedIndex = 1;
                    }

                    ddlDimentions.Enabled = false;
                }
            }

            LoadImages(ent, demoid);
        }
    }

    private void LoadImages(AdsCaptcha_ImagesEntities ent, int demoid)
    {

            var images = (from im in ent.T_IMAGES
                          join id in ent.T_DEMOS_IMAGES
                              on im.ImageID equals id.ImageID
                          where id.DemoID == demoid
                          select im).ToList();
    ...
    */
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(Request["did"]))
        {
            int demoid = Convert.ToInt32(Request["did"]);
            /*
            using (AdsCaptcha_ImagesEntities ent = new AdsCaptcha_ImagesEntities())
            {
                var demo = ent.T_DEMOS.Where(d => d.DemoID == demoid).FirstOrDefault();
    ...
    private void SaveToDBTile(AdsCaptcha_ImagesEntities ent, ref int imageID)
    {
        ...
        if ((uploadImage.PostedFile != null) && (uploadImage.PostedFile.ContentLength > 0))
        {
            ...
            try
            {
                ...
                double rotationIn1 = 5.0;
                double squareSizeIn1 = 600.0;
                double curvatureIn1 = 0.1;

                var tempBmp1 = new Bitmap(resizedCoreBmp);
                var destBMP1 = effect.ConvertImage(tempBmp1, rotationIn1, squareSizeIn1, 2, curvatureIn1);
                defMain.ImageStream = utilities.SaveJpegToStream(destBMP1, 90).ToArray();
                //defMain.ImageStream = mainImage.ImageStream;
    ...
    private void SaveToDBFrostedGlass(AdsCaptcha_ImagesEntities ent, ref int imageID)
    {
        ...
        if ((uploadImage.PostedFile != null) && (uploadImage.PostedFile.ContentLength > 0))
        {
            ...
            try
            {
                ...
                double maxRadius1 = 0.1;
                var destBMP1 = effect.ConvertImage(tempBmp1, 0.2, maxRadius1, 4);
                defMain.ImageStream = utilities.SaveJpegToStream(destBMP1, 90).ToArray();
...
*/
    protected void rptImage_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        {
            /*
            var image = e.Item.DataItem as T_IMAGES;
            if (image != null)
            {
                System.Web.UI.WebControls.Image img = e.Item.FindControl("img") as System.Web.UI.WebControls.Image;
                if (img != null)
                {
                    img.ImageUrl = "~/LandingDemo/GetImage.ashx?img=" + image.ImageID.ToString();
                }

                Button btn = e.Item.FindControl("btnDelete") as Button;
                if(btn != null)
                {
                    btn.CommandArgument = image.ImageID.ToString();
                }
            }
            */
        }
    ...
    protected void rptImage_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteImage")
        {
            if (!string.IsNullOrEmpty(Request["did"]))
            {
                ...
                using (AdsCaptcha_ImagesEntities ent = new AdsCaptcha_ImagesEntities())
                {
                    var deleteobj = ent.T_DEMOS_IMAGES.Where(d => d.ImageID == imageid && d.DemoID == demoid).FirstOrDefault();
                    if (deleteobj != null)
                    {
                        ent.DeleteObject(deleteobj);
                        ent.SaveChanges();
    ...
    {
        Response.Redirect("~/lp/" + btnPreview.CommandArgument);
    }
