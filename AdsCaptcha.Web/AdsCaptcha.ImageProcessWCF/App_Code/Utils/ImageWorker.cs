...
namespace Inqwise.AdsCaptcha.ImageProcessWCF.Utils
{
    public class ImageWorker
    {
        ...
        public void LoadImage(AdsCaptcha_ImagesEntities ent, string imageUrl, int imageType, int width, int height, ref int imgID, byte[] imageStream)
        {
            ...
            try
            {
                ...
                SaveImage(ent, img, imageType, width, height, ref imgID, imageStream);
            }
            catch (Exception)
            {
        ...
        public void LoadImageFromStream(AdsCaptcha_ImagesEntities ent, byte[] imageStream, int imageType, int width, int height, ref int imgID)
        {
            MemoryStream ms = null;
            BinaryWriter bw = null;
        ...
        private void SaveImage(AdsCaptcha_ImagesEntities ent, System.Drawing.Image coreimage, int imageType, int width, int height, ref int imgID, byte[] imageStream)
        {
            ImageUtilities utilities = new ImageUtilities();

            var resizedCoreBmp = utilities.ResizeImage(coreimage, width, height);

            var mainImage = ent.P_Images_Insert(imageType, utilities.SaveJpegToStream(resizedCoreBmp, 90).ToArray(),
                                                ".jpg", width, height, null, null, null).FirstOrDefault();

            imgID = mainImage.ImageID;
            imageStream = mainImage.ImageStream;
            ...
            try
            {
                ...
                AmazonAWS.Upload(new MemoryStream(defMain.ImageStream),
                                 string.Format(IMAGE_PATH_FORMAT, imgID, width, height, defMain.DeformationID), width,
            ...
            for (int i = 0; i < 30; i++)
            {
                ...
                try
                {
                    ...
                    AmazonAWS.Upload(new MemoryStream(def.ImageStream),
                                     string.Format(IMAGE_PATH_FORMAT, imgID, width, height, def.DeformationID), width,
                                     height, JPG_CONTENT_TYPE);
                }
                catch (Exception ex)
                {
